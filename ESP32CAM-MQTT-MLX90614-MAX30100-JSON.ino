...
#include <Wire.h>  // i2c
#include "MAX30105.h"   //biblioteca de sensor
#include "spo2_algorithm.h" // biblioteca para convertir datos del sensor
...
MAX30105 particleSensor;   //elemento sensor

#define MAX_BRIGHTNESS 255

//bloque de variables para configurar el sensor
...
int waitMax30100 = 4000; //Espera para la lectura del sensor30100
...
/*
 * Sección de comprobación de sensor colocado con monitor serial para el incio de lectura
...
// función de lectura 100 muestas
max30100First();



  timeLastMQTT = millis (); // Inicia el control de tiempo para el manejo de mqtt con conexión
  timeLastMax30100 = millis (); // Inicia el control de tiempo para el sensor
}// fin del void setup ()
...
void loop() {
 ...
 //no bloqueante para sensor 
  if (timeNow - timeLastMax30100 > waitMax30100) { // Manda los datos del sensor cada cinco segundos
    timeLastMax30100 = timeNow; // Actualización de seguimiento de tiempo

//bloque de lectura de sensor
...
//led indicador
  ...
  if (timeNow - timeLastMQTT > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    String json = "{\"hr\"="+String(heartRate)+",\"spo2\":"+String(random(18, 23))+",\"hum\":"+String(random (38,52))+"}";
